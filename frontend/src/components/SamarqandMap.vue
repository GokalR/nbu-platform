...
import { samarqandByKey } from '@/data/samarqand'
...
// Samarkand viloyat bounds (with padding) — derived from the geoBoundaries
// ADM2 extract: lon 65.14–67.55, lat 39.23–40.67.
const VIEW_W = 1000
const VIEW_H = 615
const LON_MIN = 65.05
const LON_MAX = 67.65
...
onMounted(async () => {
  try {
    const res = await fetch(`${import.meta.env.BASE_URL}samarqand-districts.geojson`)
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    const json = await res.json()
    features.value = json.features
  } catch (e) {
    error.value = e.message
    console.error('Failed to load samarqand geojson', e)
  }
})
...
const tooltipDistrict = computed(() => (hovered.value ? samarqandByKey[hovered.value] : null))
</script>
...
<template>
  ...
  >
    ...
    >
      <defs>
        <filter id="samarqand-shadow" x="-10%" y="-10%" width="120%" height="120%">
      ...
      <g>
        <path
          ...
          :style="modelValue === p.key ? 'filter: url(#samarqand-shadow)' : ''"
