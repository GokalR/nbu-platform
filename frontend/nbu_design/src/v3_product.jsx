...
function V3Product() {
  ...
  const aiFeed = [
    { t: t("v3_ai_advisor"), m: t("v3_ai_msg1"), c: NBU.emerald },
    { t: t("v3_ai_analytics"), m: t("v3_ai_msg2"), c: NBU.primary2 },
    { t: t("v3_ai_rec"), m: t("v3_ai_msg3"), c: NBU.amber },
    { t: t("v3_ai_advisor"), m: t("v3_ai_msg4"), c: NBU.red },
  ];
  ...
  return (
    <div style={{
      ...
      color: NBU.ink,
      display: "grid",
      gridTemplateColumns: "minmax(540px, 1.15fr) minmax(480px, 1fr)",
      fontFamily: "Manrope, Inter, sans-serif",
      position: "relative",
      overflow: "hidden",
    }}>
      {/* LEFT (auth form) — rendered on right */}
      <div style={{
        padding: "40px 56px",
        ...
        <div style={{ maxWidth: 520, width: "100%", position: "relative" }}>
          ...
          <h1 style={{
            ...
            {t("v3_hero_a")}<br/>
            <span style={{ color: NBU.primary }}>{t("v3_hero_b")}</span><br/>
            {t("v3_hero_c")}
          </h1>
          ...
          {f.error && (
            <div style={{ marginTop: 10, fontSize: 12, color: NBU.red, fontWeight: 600,
              ...
              <span>▲</span>{f.error === "email" ? t("err_email") : t("err_password")}
            </div>
          )}
      ...
      {/* RIGHT: rich dashboard — rendered on left */}
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              ...
              }}>{t("v3_grp_label")} +18.2% {t("v3_yoy")}</div>
            </div>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  ...
                  <div style={{
                    ...
                    fontSize: 9, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: "#A7F3D0",
                  }}>FERGANA ACTIVE</div>
                </div>
                <div style={{
                  ...
                  <img
                    src="assets/fergana_map.png"
                    alt="Ферганская долина — карта районов"
                    style={{
                ...
                <div style={{
                  ...
                  color: "rgba(255,255,255,0.55)", letterSpacing: 0.4, marginTop: 6,
                }}>
                  <span>{t("v3_stat_districts")}</span>
                  <span>{t("v3_stat_cities")}</span>
                  <span>{t("v3_stat_pop")}</span>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  { l: t("v3_kpi1_l"), v: "14,240", u: t("v3_kpi1_u"), c: "#fff" },
                  { l: t("v3_kpi2_l"), v: "2,820", u: t("v3_kpi2_u"), c: "#A7F3D0" },
                  { l: t("v3_kpi3_l"), v: "12,840", u: t("v3_kpi3_u"), c: NBU.skyFaint },
                ].map((s, i) => (
                  <div key={i} style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
            {[
              { n: t("v3_tool1"), d: t("v3_tool1d"), c: NBU.skyFaint, i: "▦" },
              { n: t("v3_tool2"), d: t("v3_tool2d"), c: NBU.emeraldLight, i: "✦" },
              { n: t("v3_tool3"), d: t("v3_tool3d"), c: "#FBBF24", i: "◐" },
              { n: t("v3_tool4"), d: t("v3_tool4d"), c: "#A78BFA", i: "⬡" },
            ].map((x, i) => (
              <div key={i} style={{
                padding: 12,
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 9, letterSpacing: 1, textTransform: "uppercase", color: NBU.muted, fontWeight: 700 }}>{t("v3_badge_l")}</div>
            <div style={{ fontSize: 14, letterSpacing: -0.3 }}>{t("v3_badge_v")}</div>
          </div>
        </div>
...
// ---- Top status badge: 5 design options, click to cycle ----
function TopStatusBadge() {
  const [idx, setIdx] = useState(() => {
    const saved = parseInt(localStorage.getItem("nbu_topbadge") || "0", 10);
    return isNaN(saved) ? 0 : saved % 5;
  });
  const next = () => {
    const n = (idx + 1) % 5;
    setIdx(n);
    localStorage.setItem("nbu_topbadge", String(n));
  };
  ...
  // Option 0 — Minimal mono pill (original refined)
  if (idx === 0) return (
    <div onClick={next} style={{ ...wrap,
      padding: "8px 14px",
      background: "rgba(255,255,255,0.08)",
      border: "1px solid rgba(255,255,255,0.18)",
      backdropFilter: "blur(12px)",
      ...
      LIVE · 14m ago
      <span style={{ opacity: 0.4, fontSize: 9 }}>01/05</span>
    </div>
  );

  // Option 1 — Data card with metric
  if (idx === 1) return (
    <div onClick={next} style={{ ...wrap,
      padding: "10px 14px",
      ...
      <div style={{
        width: 28, height: 28, borderRadius: 8, background: NBU.emerald,
        color: "#fff", display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, fontWeight: 800,
      }}>↻</div>
      <div>
        <div style={{ fontSize: 9, letterSpacing: 0.8, textTransform: "uppercase",
          color: NBU.muted, fontWeight: 700, lineHeight: 1 }}>Синхронизация</div>
        <div style={{ fontSize: 12, color: NBU.ink, fontWeight: 700, marginTop: 2 }}>
          14 мин назад <span style={{ color: NBU.emerald, fontWeight: 600 }}>· OK</span>
        </div>
      </div>
      <span style={{ fontSize: 9, color: NBU.muted, opacity: 0.5 }}>02/05</span>
    </div>
  );

  // Option 2 — Terminal / code style
  if (idx === 2) return (
    <div onClick={next} style={{ ...wrap,
      padding: "8px 12px",
      background: "rgba(0,0,0,0.55)",
      border: `1px solid ${NBU.emeraldLight}40`,
      borderRadius: 6,
      fontSize: 11, color: NBU.emeraldLight,
      display: "flex", alignItems: "center", gap: 8,
      fontFamily: "'JetBrains Mono', monospace",
    }}>
      <span style={{ color: "#fff", opacity: 0.5 }}>$</span>
      <span>sync --last</span>
      <span style={{ color: "#fff", opacity: 0.9 }}>14m</span>
      <span style={{ color: NBU.emeraldLight }}>✓</span>
      <span style={{ opacity: 0.35, color: "#fff", fontSize: 9 }}>03/05</span>
    </div>
  );

  // Option 3 — Dual-segment pill (label + value split)
  if (idx === 3) return (
    <div onClick={next} style={{ ...wrap,
      ...
      <div style={{
        padding: "8px 12px", background: NBU.ink, color: "#fff",
        letterSpacing: 0.6, textTransform: "uppercase", fontSize: 10,
        display: "flex", alignItems: "center", gap: 6,
      }}>
        <span style={{ width: 5, height: 5, borderRadius: 999, background: NBU.emeraldLight,
          boxShadow: `0 0 6px ${NBU.emeraldLight}`, animation: "pulse 2s infinite" }} />
        ОБНОВЛЕНО
      </div>
      <div style={{
        padding: "8px 12px", background: "#fff", color: NBU.ink,
        display: "flex", alignItems: "center", gap: 6,
      }}>
        14 мин назад
        <span style={{ color: NBU.muted, fontSize: 9, fontWeight: 500, opacity: 0.6 }}>04/05</span>
      </div>
    </div>
  );

  // Option 4 — Full metric bar with sparkline
  return (
    <div onClick={next} style={{ ...wrap,
      padding: "10px 14px",
      background: "rgba(0,40,85,0.7)",
      border: "1px solid rgba(127,181,230,0.3)",
      backdropFilter: "blur(12px)",
      borderRadius: 12,
      ...
      <div>
        <div style={{ fontSize: 9, letterSpacing: 0.8, textTransform: "uppercase",
          color: NBU.skyLight, fontWeight: 700, lineHeight: 1 }}>Последняя синхронизация</div>
        <div style={{ fontSize: 13, color: "#fff", fontWeight: 700, marginTop: 3,
          fontFamily: "'JetBrains Mono', monospace", letterSpacing: -0.3 }}>
          14m <span style={{ color: NBU.skyLight, opacity: 0.6, fontSize: 10 }}>/ 15m SLA</span>
        </div>
      </div>
      {/* mini sparkline */}
      <svg width="44" height="22" viewBox="0 0 44 22" style={{ opacity: 0.9 }}>
        <polyline points="0,14 6,12 12,15 18,10 24,13 30,7 36,9 42,4"
          fill="none" stroke={NBU.emeraldLight} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="42" cy="4" r="2.2" fill={NBU.emeraldLight} />
      </svg>
      <span style={{ color: "#fff", opacity: 0.35, fontSize: 9, fontWeight: 600 }}>05/05 ↻</span>
    </div>
  );
}
