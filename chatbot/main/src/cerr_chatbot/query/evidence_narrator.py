...
def build_evidence_prompt(pack: EvidencePack, brief: AnswerBrief | None = None) -> str:
    ...
    return (
        ...
        "8. NEVER emit naked classification codes the user cannot decode.\n"
        ...
        "\n"
        "STRUCTURE & FORMAT (write so the eye can scan, not as one wall of text):\n"
        "  * Start with a one-line headline answer in **bold** containing the\n"
        "    key number (e.g. **Toshkent shahrida 36,108 ta kompaniya bor.**).\n"
        "  * For rankings or comparisons of 3+ items, use a markdown table\n"
        "    with proper headers — NOT inline prose. Right-align numeric\n"
        "    columns with `---:`.\n"
        "  * For multi-section answers (e.g. ranking + analysis), use\n"
        "    `## Section title` headings. Keep to 2–3 sections max.\n"
        "  * Bullet lists for parallel items; numbered lists for ranked steps.\n"
        "  * Bold every key number and entity name in prose.\n"
        "  * Keep paragraphs short (2–3 sentences each).\n"
        "  * No hedging, no apologies, no padding.\n"
        "\n"
        "CLOSING SECTIONS (add when relevant — skip if not useful):\n"
        "  * `**Umumiy ko'rsatkichlar**` — when the answer is a top-N list or\n"
        "    filter, append totals/baselines from the context queries:\n"
        "    `Jami: 36,108 ta`, `Top-10 ulushi: 47%`, `O'rtacha qiymat: ...`.\n"
        "    Use bullets, one line each.\n"
        "  * `**Keyingi savollar**` — 2–3 short, specific follow-up questions\n"
        "    the user could ask next to drill deeper. Each should reference a\n"
        "    concrete entity from the answer above so the chatbot can run them.\n"
        "    Format as a bulleted list of plain question strings, in Uzbek.\n"
        "    Examples:\n"
        "      * \"Toshkent shahrining qaysi tumanida eng ko'p kompaniya bor?\"\n"
        "      * \"AO UzGasTrade qanday mahsulotlarni import qiladi?\"\n"
        "    Skip this section if the user asked a trivial question or the\n"
        "    answer is already exhaustive.\n"
        "\n"
        "OUTPUT CONTRACT:\n"
