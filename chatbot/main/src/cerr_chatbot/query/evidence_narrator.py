...
def build_evidence_prompt(pack: EvidencePack, brief: AnswerBrief | None = None) -> str:
    ...
    return (
        ...
        "7b. LEAD WITH THE ASKED METRIC. If the user's question explicitly\n"
        "    names a single specific metric (ishsizlik, aholi, reyting,\n"
        "    biznes, tibbiyot masofa, asfalt yo'l, muammoli kredit, kam\n"
        "    ta'minlangan oila, etc.) — the bold HEADLINE must be that\n"
        "    metric with its bolded number. NEVER lead with population\n"
        "    or general regional stats when something else was asked.\n"
        "    BAD (user asked 'ishsizlik'):\n"
        "      **Toshkent bo'yicha asosiy ko'rsatkichlar: aholi 3,177,589 ...**\n"
        "      (next paragraph) Ishsizlar soni 13,686 ...\n"
        "    GOOD (user asked 'ishsizlik'):\n"
        "      **Toshkent shahrida ishsizlik 13,686 nafar,\n"
        "       Toshkent viloyatida 30,589 nafar.**\n"
        "      (next paragraph) Aholi nisbatlari, share-of-population,\n"
        "      district breakdown, etc.\n"
        "    The first sentence of every answer must answer THE specific\n"
        "    question the user asked, not the closest profile dump.\n"
        "8. Bold markdown `**X**` is FRAGILE — four things will make it\n"
        "   render as literal asterisks or split into the wrong words:\n"
        "   (a) `**80%**i` — closing `**` followed directly by a letter.\n"
        "       Always put a space: `**80%** ulushi`, `**80%** ga`.\n"
        "   (b) `** 5 tasi**` — opening `**` followed by whitespace. The\n"
        "       opening marker must touch the first real character. Write\n"
        "       `**5 tasi**`, never `** 5 tasi**`.\n"
        "   (c) `**5 tasi **` — closing `**` preceded by whitespace. Same\n"
        "       rule on the other side: `**5 tasi**`, never `**5 tasi **`.\n"
        "   (d) NEVER split a single visual phrase across two bold spans.\n"
        "       BAD:  `**Samarqandda ... keladigan** 8 ta**yetkazib ... bor.**`\n"
        "             ↑ two `**...**` pairs glued together — `8 ta` falls\n"
        "             out of the bold and looks like a typo.\n"
        "       GOOD: `**Samarqandda ichimliklarga oid qat'iy mos keladigan\n"
        "             8 ta yetkazib beruvchi / sotuvchi ko'rindi.**`\n"
        "             ↑ ONE continuous `**...**` covering the whole headline.\n"
        "       The HEADLINE LINE (first sentence of every answer) MUST be\n"
        "       exactly one bold span end-to-end, including all numbers and\n"
        "       entity names that belong to that sentence.\n"
        "   The same four rules apply to single-asterisk italics `*X*`, and\n"
        "   to Russian, Uzbek Cyrillic, and English suffixes alike.\n"
        "   Quick reference: the asterisks must hug the text tightly inside,\n"
        "   the next char outside must be whitespace or punctuation, and a\n"
        "   single phrase never carries more than one `**...**` pair.\n"
