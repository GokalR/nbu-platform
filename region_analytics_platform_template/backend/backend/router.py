...
at startup, then for each incoming question substring-matches against that index in BOTH
Cyrillic and Latin forms. Returns the list of viloyats that should receive the question.
...
from config import RAG_DIR, VILOYATS


# Cyrillic → Latin Uzbek transliteration map.
_CYR2LAT = {
...
# Apostrophe variants seen in the wild.
_APOS_VARIANTS = ["'", "'", "ʼ", "ʻ", "`", "´"]
...
def build_index() -> list[dict[str, Any]]:
    """Build (or return cached) per-viloyat name index."""
    global _INDEX
    ...
    for v in VILOYATS:
        ...
        # Viloyat-level names + short aliases.
...
def route(question: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    If no name matches in the question → returns ALL 14 viloyats (assume global query).
    """
    idx = build_index()
    ...
    for entry in idx:
        for name in entry["names"]:
            if name in q:
                ...
                break  # one match per viloyat is enough
