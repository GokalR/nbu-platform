...
На вход получаешь JSON с данными анкеты предпринимателя плюс:
  ...
  • computedFinancials — фактические числа. ИСПОЛЬЗУЙ ИМЕННО ЭТИ СЛЕДУЮЩИЕ ПОЛЯ:
      - monthlyRevenueUzs              — месячная выручка
      - monthlyOpCosts                 — операционные расходы (БЕЗ обслуживания долга, ВКЛЮЧАЯ налоги)
      - monthlyEbitda                  — EBITDA (revenue − opCosts)
      - ebitdaMarginPct                — EBITDA маржа в %
      - loanSteadyStatePayment         — платёж по кредиту в стационарном режиме (после льготного периода)
      - taxes                          — структура налогов
    В обсуждении долга ВСЕГДА используй loanSteadyStatePayment (не avg-12m), потому что
    это реальная нагрузка после грейс-периода — то, что банк рассматривает как обязательство.
  • computedCreditScoreV2 — детерминистический скоринг (5 критериев, 0-100). СТРОГО
    ОПИРАЙСЯ НА ЭТИ ЧИСЛА в narrative и risks:
      - verdict / total                — итоговая оценка (НЕ ПЕРЕСЧИТЫВАЙ)
      - criteria.dscrSteadyState.value — DSCR в стационарном режиме (то, что в таблице)
      - criteria.equityShare.value     — доля собственных средств в %
      - criteria.profitability.value   — EBITDA маржа в %
      - criteria.realism.value         — число предупреждений по реалистичности
      - criteria.resilience.value      — DSCR в худшем сценарии (стресс или год-1)
      - stress.dscr                    — DSCR в стресс-сценарии (выручка −20%, расходы +10%)
      - year1Ramp.dscr                 — DSCR в среднем по году 1 (с ramp выходом на проектную мощность)
      - plausibilityFlags              — массив сработавших проверок реалистичности
        (каждый объект имеет message_ru, value, expected — цитируй буквально в risks)
  • computedCreditScore — устаревшие V1 ratios. НЕ ИСПОЛЬЗУЙ если есть computedCreditScoreV2.
...

{
  ...
  "executiveSummary": "1 абзац (3-4 предложения, ≤500 символов) — для руководителя банка. Цитируй конкретные V2-числа: вердикт + total, DSCR (steady-state), EBITDA маржу, equity share, и стресс-DSCR если он < 1.",
  ...
  "teamAssessment": "1 предложение — адекватен ли штат объёмам. Если в plausibilityFlags есть revenue_per_employee_outside_band — упомяни это конкретно.",
  "financialsAssessment": "2 предложения — используй EBITDA маржу (criteria.profitability.value), DSCR steady-state (criteria.dscrSteadyState.value) и стресс-DSCR (stress.dscr). НЕ используй термины 'операционная маржа' или 'чистая маржа' или 'costToRevenue' — это V1 терминология.",
...
4. risks — РОВНО 3 объекта. Каждый ссылается на конкретное поле computedFinancials или
   computedCreditScoreV2.criteria. Если в plausibilityFlags есть нерешённые предупреждения,
   ОДИН из 3 рисков ОБЯЗАН цитировать самый серьёзный из них (high severity первый).
...
ВАЖНО — СОГЛАСОВАННОСТЬ С ТАБЛИЦЕЙ СКОРИНГА:
В narrative ты ОБЯЗАН цитировать V2 числа, которые видит пользователь в таблице:
  • DSCR в narrative = computedCreditScoreV2.criteria.dscrSteadyState.value (НЕ V1 11.88x)
  • маржа в narrative = computedCreditScoreV2.criteria.profitability.value (EBITDA маржа,
    НЕ V1 «операционная маржа» или «чистая маржа»)
  • стресс DSCR = stress.dscr; год-1 DSCR = year1Ramp.dscr
  • equity = criteria.equityShare.value
Если назовёшь 11.88x когда таблица показывает 7.86x — пользователь увидит противоречие.
Это критическая ошибка целостности. Используй ТОЛЬКО V2 данные.
"""
...
Кириш JSON да тадбиркор анкетаси ҳамда:
  • candidateProducts — 3 та НБУ кредит маҳсулоти; сен 1-2 тасини `id` бўйича танлайсан
    ва асос ёзасан. СТАВКА, СУММА, МУДДАТ, МАҚСАД — серверда каталогдан олинади.
  • computedFinancials — серверда ҳисобланган аниқ рақамлар. ФАҚАТ ҚУЙИДАГИ МАЙДОНЛАРНИ
    ИШЛАТ:
      - monthlyRevenueUzs              — ойлик даромад
      - monthlyOpCosts                 — операцион харажат (қарз тўловларсиз, солиқлар билан)
      - monthlyEbitda                  — EBITDA (revenue − opCosts)
      - ebitdaMarginPct                — EBITDA маржа %
      - loanSteadyStatePayment         — стационар режимдаги кредит тўлови
        (имтиёзли давр тугагандан кейин)
      - taxes                          — солиқ тузилмаси
    Қарз тўғрисидаги муҳокамада ҲАМИША loanSteadyStatePayment ни ишлат — бу банк
    кўриб чиқадиган реал юк, year-1 average эмас.
  • computedCreditScoreV2 — детерминистик скоринг (5 мезон, 0-100). АНИҚ БУ
    РАҚАМЛАРНИ narrative ва risks да цитата қил:
      - verdict / total                — якуний баҳо (ҚАЙТА ҲИСОБЛАМА)
      - criteria.dscrSteadyState.value — стационар DSCR (жадвалда кўрсатилган)
      - criteria.equityShare.value     — ўз маблағ улуши %
      - criteria.profitability.value   — EBITDA маржа %
      - criteria.realism.value         — реалликка оид огоҳлантиришлар сони
      - criteria.resilience.value      — энг ёмон стсенарийдаги DSCR
      - stress.dscr                    — стресс-DSCR (даромад −20%, харажат +10%)
      - year1Ramp.dscr                 — 1-йил ўртача DSCR (ramp билан)
      - plausibilityFlags              — фаоллашган реаллик текширувлари рўйхати
        (ҳар бир объектда message_ru, value, expected — risks да тўғридан-тўғри цитата қил)
  • computedCreditScore — эски V1 ratios. computedCreditScoreV2 бўлса, ИШЛАТМА.
...

{
  ...
  "executiveSummary": "1 абзац (3-4 жумла, ≤500 белги) — банк раҳбари учун. V2 рақамларини цитата қил: вердикт + total, DSCR (steady-state), EBITDA маржа, equity share, стресс-DSCR (агар < 1).",
  ...
  "teamAssessment": "1 жумла. Агар plausibilityFlags да revenue_per_employee_outside_band бўлса, шуни конкрет тилга ол.",
  "financialsAssessment": "2 жумла. EBITDA маржа (criteria.profitability.value), DSCR steady-state (criteria.dscrSteadyState.value), стресс-DSCR (stress.dscr) ишлат. V1 терминологияси: «операцион маржа», «соф маржа», «costToRevenue» — ИШЛАТМА.",
...
4. risks — АЙНАН 3 объект. Ҳар бири computedFinancials ёки computedCreditScoreV2.criteria
   даги аниқ рақамга ишора қилсин. Агар plausibilityFlags да фаол огоҳлантиришлар бўлса,
   3 risk дан БИРИ энг жиддийсини (high severity) айнан тилга олиши шарт.
...
МУҲИМ — СКОРИНГ ЖАДВАЛИ БИЛАН МУВОФИҚЛИК:
narrative да фойдаланувчи жадвалда кўрган V2 рақамларини цитата қилиш ШАРТ:
  • DSCR = computedCreditScoreV2.criteria.dscrSteadyState.value (V1 11.88x ЭМАС)
  • маржа = computedCreditScoreV2.criteria.profitability.value (EBITDA маржа,
    «операцион маржа» ёки «соф маржа» эмас)
  • стресс DSCR = stress.dscr; 1-йил DSCR = year1Ramp.dscr
Жадвалда 7.86x кўрсатилган, сен 11.88x десанг — фойдаланувчи зиддиятни кўради.
"""
...
def _build_prompt_payload(
    *,
    ...
    When V2 scoring is available (passed through `historical_financials.scoreV2`),
    the payload exposes V2-flavor data (steady-state DSCR, EBITDA margin,
    year-1 ramp DSCR, stress DSCR, plausibility flags) so the LLM's narrative
    cites the same numbers that appear in the V2 result-page table. V1 data
    stays in the payload for backward compatibility but the prompts now
    instruct the model to prefer V2 fields.
    """
    payload: dict[str, Any] = {
        ...
        "computedFinancials": {
            ...
            # V2 primary numbers — these match the V2 credit-scoring table
            # shown to the user. The LLM should use these in the narrative.
            "monthlyOpCosts": baseline["monthlyCosts"].get("operating", 0),
            "monthlyEbitda": (baseline["revenue"]["monthlyRevenueUzs"]
                              - baseline["monthlyCosts"].get("operating", 0)),
            "ebitdaMarginPct": baseline["marginsAvg12m"].get("ebitdaMarginPct", 0),
            "loanSteadyStatePayment": baseline["loan"].get(
                "steadyStateMonthlyPayment", 0),
            "taxes": baseline.get("taxes", {}),
            # V1 fields — kept for backward compat with old prompts/plans.
            # Prefer the V2 fields above for narrative.
    ...
    if historical_financials:
        ...
        # V1 (legacy) — leave for back-compat
        score_v1 = historical_financials.get("score") or {}
        if score_v1.get("verdict"):
            payload["computedCreditScore"] = {
                "verdict": score_v1.get("verdict"),
                "percent": score_v1.get("percent"),
                "ratios": {k: {"value": v.get("value"), "unit": v.get("unit"),
                               "score": v.get("score")}
                           for k, v in (score_v1.get("ratios") or {}).items()},
            }

        # V2 (preferred) — when present, this is the data shown to the user;
        # narrative should cite these specific numbers.
        score_v2 = historical_financials.get("scoreV2") or {}
        if score_v2.get("verdict") and score_v2.get("total") is not None:
            criteria = score_v2.get("criteria") or {}
            payload["computedCreditScoreV2"] = {
                "verdict": score_v2.get("verdict"),
                "total": score_v2.get("total"),
                "industry": score_v2.get("industry", {}),
                "criteria": {
                    k: {"value": (criteria.get(k) or {}).get("value"),
                        "points": (criteria.get(k) or {}).get("points"),
                        "label": (criteria.get(k) or {}).get("label")}
                    for k in ("dscrSteadyState", "equityShare", "profitability",
                              "realism", "resilience")
                    if k in criteria
                },
                "stress": score_v2.get("stress", {}),
                "year1Ramp": score_v2.get("year1Ramp", {}),
                # Surface plausibility flags so the LLM can cite them directly
                # in risks / executive summary instead of inventing concerns.
                "plausibilityFlags": (criteria.get("realism") or {}).get("flags") or [],
            }
    return payload
